...
keys = str(list(london_co[user_key].keys()))
keys = keys.replace("'", "").replace('[', '(').replace(']', ')')
print('Enter parameter name ', keys)
user_param = input(':').lower()
print(london_co[user_key].get(user_param, 'Такого параметра нет'))
